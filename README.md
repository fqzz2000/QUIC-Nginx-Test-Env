...
bash# On the load balancer node
...
bash# Create the data directory if it doesn't exist
mkdir -p data
...
bashdocker build -t nginx-backend -f backend/Dockerfile ./backend

Start the backend server:
bashdocker run --name nginx-backend \
  --network host \
...
bashdocker build -t nginx-frontend-http3 -f frontend/Dockerfile ./frontend

Start the HTTP3 load balancer:
bashdocker run --name nginx-frontend-http3 \
  --network host \
...
bash# Test HTTP3 from client to load balancer 
...
apt install -y git build-essential nghttp2 libnghttp2-dev libssl-dev

# Get curl source
git clone https://github.com/curl/curl.git
cd curl

# Configure with HTTP3 support
./buildconf
...
make install
Performance Testing
You can use tools like curl -w to measure download times and compare:
bashcurl -k --http3 -w "Time: %{time_total}s\n" https://node1.example.com:8443/test_file_5gb.bin -o /dev/null
Stopping Services
bash# On load balancer node
